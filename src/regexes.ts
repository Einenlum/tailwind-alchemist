...
const allPseudoClasses = TAILWIND_PSEUDO_CLASSES.join('|');
const allColorUtilities = TAILWIND_COLOR_UTILITIES.join('|');

export const FULL_RANGE_REGEX = new RegExp(
  `\\b(?:(?:${allPseudoClasses}):)*(?:${allColorUtilities})-(?<color>${allColors})\\b`,
  'g',
);

export const getTailwindColorRegex = (color: string) => {
  return new RegExp(
    `\\b(?:(?:${allPseudoClasses}):)*(?:${allColorUtilities})-(?<color>${color})\\b`,
    'g',
  );
};
